...
export default function ActiveStatusList(props) {
  ...
  const getAllStatus = async () => {
    try {
      await axios.get(`${urls_v1.status.get_all}?PageNumber=${pageNumber}&PageSize=${rowsPerPage}`).then(({ data }) => {
        if (data.succeeded) {
  ...
  return (
    <>
      <div className={classes.mainDiv}>
        ...
        {userPermission?.includes(statusPermissions.CREATE) && (
          <Button
            size="small"
            onClick={() => {
              setOpenPopup(true);
