...
export default function PurchaseTypeList() {
  ...
  const getAllPurchaseTypes = async () => {
    try {
      await axios
        .get(
          `${urls_v1.purchaseType.get_all_archived}?PageNumber=${pageNumber}&PageSize=${rowsPerPage}&SortedColumn=${sortedColumn}&SortedBy=${sortedBy}`
        )
        .then(res => {
  ...
  const onActivate = row => {
    ...
    const data = { ...row };
    data.isActive = true;
    axios.put(`${urls_v1.purchaseType.put}/${data.key}`, data).then(({ data }) => {
      if (data.succeeded) {
        toastAlerts('success', 'Packaging-Type activated');
        getAllPurchaseTypes();
      } else {
                    ...
                    <Button
                      ...
                      onClick={() => {
                        setConfirmationDialog({
                          isOpen: true,
                          heading: 'Packaging Type Activation',
                          title: 'Are you sure to active this Packaging Type?',
                          onConfirm: () => {
                            onActivate(record);
                          }
                        });
