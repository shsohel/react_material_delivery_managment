...
import { Box, Button, CircularProgress, Grid, makeStyles, Paper, TextField } from '@material-ui/core';
...
export default function RoleForm(props) {
  ...
  const handleSubmit = e => {
    ...
    if (data.name && data.description && data.permissions.length) {
      axios
        .post(`${urls_v1.account.roles.post}`, data)
        .then(res => {
      ...
      toastAlerts('warning', 'Please fill all the information');
    }
  };
                ...
                <Box display="flex">
                  ...
                  <Box ml={2}>
                    <NavLink to="/accounts/role">
                      <Button size="small" color="primary" type="submit" variant="outlined">
                        Cancel
                      </Button>
                    </NavLink>
                  </Box>
