...
import { ConfirmIcon, ReviewIcon, SettingsIcon, ViewIcon } from '@jumbo/controls/ActionButtons';
import { sweetAlerts, toastAlerts } from '@jumbo/utils/alerts';
import {
  Backdrop,
  Box,
  Button,
...
import { Add, FilterListSharp, Search, Sync } from '@material-ui/icons';
...
export default function OrderList(props) {
  ...
  const getStatus = record => {
    const { isAccepted, isRejected, isConfirmed, isCompleted, orderNumber } = record;
    if (isRejected) {
      return {
        text: 'Rejected',
        bgcolor: '#E00930'
  ...
  const onSync = () => {
    ...
    axios
      ...
      .then(({ data }) => {
        if (data.succeeded) {
          setOpen(false);
          sweetAlerts('success', 'Succeded', data.message);
          getAllOrders();
          // props.history.replace('/orders/list');
        } else {
          sweetAlerts('error', 'Not Succeded', data.message);
          props.history.replace('/orders/list');
        }
      })
      .catch(({ response }) => {
        sweetAlerts('error', 'Error', 'THere is an error!!!. Please contact with admin!!!');
        props.history.replace('/orders/list');
      });
  };
  ...
  const onReview = (id, key) => {
    const body = {
      id,
      key
    };
    axios
      .put(`${urls_v1.order.review}/${key}`, body)
      .then(({ data }) => {
        if (data.succeeded) {
      ...
      .catch(({ response }) => {
        toastAlerts('error', 'There is a problem!!!');
      });
  };
  //#endregion
                ...
                <TableRow key={record.id}>
                  <TableCell>
                    {record.orderNumber ? (
                      ...
                      <Box className={classes.badgeRoot} component="span" bgcolor="#AAAAAA">
                        Requested
                      </Box>
                    )}
                  </TableCell>
                          ...
                          <ViewIcon
                            ...
                            onClick={() => {
                              props.history.replace({
                                ...
                                search: `?ordernumber=${record.orderNumber}`
                              });
                            }}
                          />
                        )
                  ...
                  <TableCell>
                    <>
                      {/* {!record.orderNumber && !record.isAccepted
          ...
          <Grid xs item>
            {userPermission?.includes(orders.CREATE) && (
              <NavLink to="/orders/new">
                <Button className={classes.btnNew} variant="outlined" color="primary" size="small" endIcon={<Add />}>
                  New
                </Button>
              </NavLink>
            )}
