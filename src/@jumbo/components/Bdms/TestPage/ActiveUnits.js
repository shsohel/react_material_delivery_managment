...
import UnitForm from '../Settings/Units/UnitForm';
...
export default function ActiveUnitList() {
  ...
  const getAllUnits = async () => {
    try {
      await axios
        .get(
          `${urls_v1.unit.get_all}?PageNumber=${pageNumber}&PageSize=${rowsPerPage}&SortedColumn=${sortedColumn}&SortedBy=${sortedBy}`
        )
        .then(res => {
  ...
  const saveUnit = (unit, resetForm) => {
    if (!unit.key) {
      axios.post(`${urls_v1.unit.post}`, unit).then(({ data }) => {
        if (data.succeeded) {
          toastAlerts('success', 'New Unit Successfully Added!!!');
          getAllUnits();
        } else {
      ...
      axios.put(`${urls_v1.unit.put}/${unit.key}`, unit).then(({ data }) => {
        if (data.succeeded) {
  ...
  const onDelete = key => {
    ...
    axios.delete(`${urls_v1.unit.delete}/${key}`).then(({ data }) => {
      if (data.succeeded) {
            ...
            <TableBody>
              {records.map(record => (
                <TableRow key={record.id}>
                  ...
                  <TableCell>
                    <>
                      {userPermission?.includes(units.EDIT) && (
                        <EditIcon
                          title="Edit Unit"
                          placement="top"
                          onClick={() => {
                      ...
                      {userPermission?.includes(units.DELETE) && (
                        <DeleteIcon
                          title="Delete Unit"
                          placement="top"
                          onClick={() => {
  ...
  return (
    <PageContainer>
      <div className={classes.mainDiv}>
        {userPermission?.includes(units.CREATE) && (
          <Button
            onClick={() => {
              setOpenPopup(true);
              setRecordForEdit(null);
        ...
        <Controls.Popup title="Unit Form" openPopup={openPopup} setOpenPopup={setOpenPopup}>
          <UnitForm saveUnit={saveUnit} recordForEdit={recordForEdit} />
        </Controls.Popup>
